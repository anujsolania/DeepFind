                    ...
                    >
                      ...
                      {message.role === "User" ? (
                        ...
                        <ReactMarkdown
                          ...
                          components={{
                            h1: ({ node, ...props }) => <h1 className="text-xl font-extrabold text-zinc-50 mt-5 mb-3" {...props} />,
                            h2: ({ node, ...props }) => <h2 className="text-lg font-bold text-zinc-100 mt-4 mb-2.5" {...props} />,
                            h3: ({ node, ...props }) => <h3 className="text-base font-bold text-zinc-200 mt-3.5 mb-2" {...props} />,
                            h4: ({ node, ...props }) => <h4 className="text-sm font-semibold text-zinc-300 mt-3 mb-1.5" {...props} />,
                            p: ({ node, ...props }) => <p className="my-2 text-zinc-300 leading-relaxed" {...props} />,
                            ...
                            table: ({ node, ...props }) => (
                              <div className="overflow-x-auto my-4 rounded-xl border border-zinc-800 bg-[#0e1017]">
                                <table className="w-full border-collapse text-left text-xs" {...props} />
                              </div>
                            ),
                            thead: ({ node, ...props }) => <thead className="bg-[#161822] border-b border-zinc-800 text-zinc-400 font-semibold" {...props} />,
                            ...
                            th: ({ node, ...props }) => <th className="px-4 py-3 font-semibold text-emerald-400" {...props} />,
                            td: ({ node, ...props }) => <td className="px-4 py-2.5 text-zinc-300" {...props} />,
                            code: ({ node, className, children, ...props }) => {
                              const match = /language-(\w+)/.exec(className || '');
                              return match ? (
                                <pre className="bg-[#0e1017] border border-zinc-800/80 p-3.5 rounded-xl overflow-x-auto my-3.5 text-xs font-mono text-zinc-200">
                                  <code className={className} {...props}>{children}</code>
                                </pre>
                              ) : (
                                <code className="bg-zinc-900/90 border border-zinc-800 px-1.5 py-0.5 rounded-md font-mono text-xs text-emerald-400 font-semibold" {...props}>{children}</code>
                              );
                            }
                          }}
                        >
